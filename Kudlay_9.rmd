...
Type7 <- rep(0, length(Glass$Type)) # создание вектора Type7
Glass <- cbind(Glass, Type7)        # присоединение Type7 к фрейму Glass

# замена в переменной Type: если Type = 2 означает наличие признака (1), остальные - отсутствие(0)
for(i in 1:length(Glass$Type)) {if (Glass$Type[i] == 2) {Glass$Type7[i] = 1}}

# определение долей
table(Glass$Type7) / sum(table(Glass$Type7))
```

Доля наименьшего класса, в данном случае 0.355, это ошибка нулевого классификатора: если бы мы прогнозировали Type = 7 для всех наблюдений, ровно в такой доле случаев мы бы ошиблись. Точность моделей целесообразно будет сравнивать с этой величиной.
...
set.seed(my.seed)
inTrain <- sample(seq_along(Glass$Type7),
...
# легенда
legend('bottomright', names <-  c('Лог. кривая', 'LDA', 'SVM (обр.)'), lty = 1, col = c('blue', 'red', 'green'))
```
